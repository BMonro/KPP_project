...
export default function Menu() {
    ...
    return (
        <section className="pb-8 h-full pt-12 px-20 flex flex-col">
            ...
            <button className="  bg-green py-2 px-5 mb-6 w-[30%] text-[40px] text-black rounded-[21px] shadow-add-button self-end ">Add Pizza</button>
...
export function Item({ title, time, money, img, index, handleDeleteItem }) {
    ...
    return (
        <div className=" bg-[#F1DACE] relative h-[150px] flex w-full items-center mb-9 border-[#708360] border-[4px] rounded-[21px]">
            ...
            <button onClick={() => handleDeleteItem(index)} className="rounded-[10px] flex items-center justify-center h-full pl-3 pr-2 bg-[#708360]"> <Image src={deleteImg} alt="img" /></button>
        </div>
    );
}
